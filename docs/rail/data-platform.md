...
Committed JSON evidence records a real 2026-07-31 run, an unchanged-hash rerun, a duplicate rejection integration test and forced recovery test. The scheduled/manual `rail-data-platform.yml` workflow resolves seven completed Helsinki calendar dates, runs the real Java 17 Spark/Delta path, reconciles and validates the compact publication, retains execution evidence for 30 days, and only then atomically advances `rail-publications`. That branch is the small persistent publication surface; GitHub-hosted runners and their local Delta directories remain stateless between runs and are safely rebuilt/reacquired each day. Full Spark integration tests remain in the `rail-lakehouse` code-CI job, while the daily path runs the actual bounded Spark pipeline plus lightweight publication-contract tests. Generated Delta data stays below ignored `data/`; no source payload or invented platform screenshot is committed. Microsoft Fabric and native Power BI execution are not claimed because they require the owner's tenant. Local/CI Delta execution is real.
